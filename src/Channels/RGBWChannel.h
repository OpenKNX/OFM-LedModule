...
class RGBWChannel : public LightChannel
{
  public:
    RGBWChannel(uint8_t channel_number, HWDimmer* pDimmer, uint8_t hwChannels[4]);
    ...
    // loop() drives only R/G/B (the white pin is not written), so the footprint mirrors RGB.
    float budgetFootprint() override
    {
        ...
        return (float)((uint32_t)rgb.Red() + rgb.Green() + rgb.Blue()) / (float)VALUE_KNX_COUNT;
    }
#endif
  ...
  private:
    ...
    StatusValueState _statusRgb;
    StatusValueState _statusHsv;
