...
void TWChannel::loop()
{
  ...
  if (  (      ( _brightness.getStairTime()  + (ParamLED_SC_StairCaseTimer_*1000) )   <=   millis()      )  && _brightness.getStairTrigger()   )
      ...
      if ( ! TW_night() )
      { if ( ParamLED_TW_StartupBehavior_ ) { _brightness.setLastOnValue( _brightness.value() ) ; }
  ...
  else if(!_isLocked)
  {
    switch (relKO)
    {
          ...
          if ( tmpu16 >= 0x09 )
            {
              ...
              if( !TW_night() ) {_brightness.setTargetValue( ParamLED_TW_BrighnessMaxDay_   , millis() , ParamLED_TW_LightDimmTimeRel_);}
              if(  TW_night() ) {_brightness.setTargetValue( ParamLED_TW_BrighnessMaxNight_ , millis() , ParamLED_TW_LightDimmTimeRel_);}
            }
          if ( tmpu16 > 0x00 && tmpu16 < 0x08 )
            {
              logDebugP("rel_dimming down");
              if( !TW_night() ) {_brightness.setTargetValue( ParamLED_TW_BrighnessMin_ , millis() , ParamLED_TW_LightDimmTimeRel_);}
              if(  TW_night() ) {_brightness.setTargetValue( ParamLED_TW_BrighnessMin_ , millis() , ParamLED_TW_LightDimmTimeRel_);}
            }
          if ( tmpu16 == 0x00 || tmpu16 == 0x08)
            {
      ...
      case LED_TW_KoColorTemperature_:
        ...
        if ( ! TW_night() ) {   _colorTemperature.setTargetValue(colorTemp, millis(), ParamLED_TW_LightDimmTimeDayON_    );  }
        if (   TW_night() ) {   _colorTemperature.setTargetValue(colorTemp, millis(), ParamLED_TW_LightDimmTimeNightON_  );  }
        break;
      ...
      case LED_TW_KoNight_:
          ...
          if (_tw_night) {logDebugP("es ist nacht");}
          if (!_tw_night) {logDebugP("es ist tag");}
      break;
