#include "RGBWChannel.h"
#include <knx.h>

RGBWChannel::RGBWChannel(uint8_t index, HWDimmer* pDimmer, uint8_t hwChannels[4])
    : LightChannel(index, pDimmer, hwChannels, 4), _hue(0, 0, H_PART - 1), _saturation(0, 0, VAL_RANGE)
{
    logInfoP("Trying to read Config from KNX...");
    logHexInfoP((uint8_t*)knx.paramData(LED_RGB_ParamCalcIndex(LED_RGBW_ChSceneA_Type)), 8);
    ...
    KoLED_RGBW_ChStateOnOff.value(false, DPT_State);
    KoLED_RGBW_ChBrightnessStatus.value((uint16_t)(_brightness.value() / VALUE_KNX_MULTIPLY), DPT_Scaling);
    KoLED_RGBW_ChColorTemperatureStatus.valueNoSend((uint16_t)0, Dpt(7, 600));
    KoLED_RGBW_ChHSVStatus.valueNoSend(hsv.toUint32(), DPT_Colour_RGB);
...
const std::string RGBWChannel::name()
{
    return "RGBWChannel";
}

void RGBWChannel::update()
{
    ...
    if (ParamLED_RGBW_ChStatusOnOffSend)
    {
        if ((bool)KoLED_RGBW_ChStateOnOff.value(DPT_State) != stateOn)
            KoLED_RGBW_ChStateOnOff.value(stateOn, DPT_State);

        if (ParamLED_RGBW_ChStatusOnOffTimeMS > 0 && delayCheckMillis(_statusSendOnOffTimer, ParamLED_RGBW_ChStatusOnOffTimeMS))
        {
            KoLED_RGBW_ChStateOnOff.value(stateOn, DPT_State);
    ...
    if (ParamLED_RGBW_ChStatusBrightnessSend)
    {
        ...
        if (brightnessDifference > 0 &&
            (uint8_t)KoLED_RGBW_ChBrightnessStatus.value(DPT_Scaling) != koValue)
        {
            if (_lastBrightnessLevel > 0 && brightnessDifference >= _lastBrightnessLevel * ParamLED_RGBW_ChStatusBrightnessMinChangePercent / 100.0f &&
                brightnessDifference >= ParamLED_RGBW_ChStatusBrightnessMinChangeAbsolute)
                KoLED_RGBW_ChBrightnessStatus.value(koValue, DPT_Scaling);
            else
                KoLED_RGBW_ChBrightnessStatus.valueNoSend(koValue, DPT_Scaling);
        }

        if (ParamLED_RGBW_ChStatusBrightnessTimeMS > 0 && delayCheckMillis(_statusSendBrightnessTimer, ParamLED_RGBW_ChStatusBrightnessTimeMS))
        {
            KoLED_RGBW_ChBrightnessStatus.value(koValue, DPT_Scaling);
    ...
    if (ParamLED_RGBW_ChStatusTempSend)
    {
        ...
        if (colorDifference > 0 &&
            (uint16_t)KoLED_RGBW_ChColorTemperatureStatus.value(Dpt(7, 600)) != tmpColor)
        {
            if (stateOn &&
                (_lastColorTemp > 0 && colorDifference >= _lastColorTemp * ParamLED_RGBW_ChStatusTempMinChangePercent / 100.0f &&
                 colorDifference >= ParamLED_RGBW_ChStatusTempMinChangeAbsolute))
                KoLED_RGBW_ChColorTemperatureStatus.value(tmpColor, Dpt(7, 600));
            else
                KoLED_RGBW_ChColorTemperatureStatus.valueNoSend(tmpColor, Dpt(7, 600));
        }

        if (ParamLED_RGBW_ChStatusTempTimeMS > 0 && delayCheckMillis(_statusSendTemperaturTimer, ParamLED_RGBW_ChStatusTempTimeMS))
        {
            if (stateOn)
                KoLED_RGBW_ChColorTemperatureStatus.value(tmpColor, Dpt(7, 600));
            else
    ...
    if (ParamLED_RGBW_ChStatusRGBSend)
    {
        if (_lastHueValue != tmpHue || _lastSatValue != tmpSat ||
            ParamLED_RGBW_ChStatusRGBTimeMS > 0 && delayCheckMillis(_statusSendRgbTimer, ParamLED_RGBW_ChStatusRGBTimeMS))
        {
            if (stateOn)
                KoLED_RGBW_ChRGBStatus.value(Colors::hsv2rgb(hsv).toUint32(), DPT_Colour_RGB);
            else
                KoLED_RGBW_ChRGBStatus.valueNoSend(Colors::hsv2rgb(hsv).toUint32(), DPT_Colour_RGB);
    ...
    if (ParamLED_RGBW_ChStatusHSVSend)
    {
        if (_lastHueValue != tmpHue || _lastSatValue != tmpSat ||
            ParamLED_RGBW_ChStatusHSVTimeMS > 0 && delayCheckMillis(_statusSendHsvTimer, ParamLED_RGBW_ChStatusHSVTimeMS))
        {
            if (stateOn)
                KoLED_RGBW_ChHSVStatus.value(hsv.toUint32(), DPT_Colour_RGB);
            else
                KoLED_RGBW_ChHSVStatus.valueNoSend(hsv.toUint32(), DPT_Colour_RGB);
    ...
    processSendValue(KoLED_RGBW_ChCurrent, DPT_Value_Electric_Current, ParamLED_RGBW_ChCurrentSend, ParamLED_RGBW_ChCurrentSendMinChangePercent, ParamLED_RGBW_ChCurrentSendMinChangeAbsolute, ParamLED_RGBW_ChCurrentSendCyclicTimeMS, _currentCyclicSendTimer, _lastSentCurrent, current, 1000);
    ...
    processSendValue(KoLED_RGBW_ChPower, DPT_Value_Power, ParamLED_RGBW_ChPowerSend, ParamLED_RGBW_ChPowerSendMinChangePercent, ParamLED_RGBW_ChPowerSendMinChangeAbsolute, ParamLED_RGBW_ChPowerSendCyclicTimeMS, _powerCyclicSendTimer, _lastSentPower, power);

    processDeviceProtection(KoLED_RGBW_ChDeviceProtConstCurrent, KoLED_RGBW_ChDeviceProtOverload, ParamLED_RGBW_ChDeviceProtActive, ParamLED_RGBW_ChDeviceProtConstCurrent, ParamLED_RGBW_ChDeviceProtOverloadPercent, ParamLED_RGBW_ChDeviceProtOverloadTimeMS, _deviceProtOverloadTimer, ParamLED_RGBW_ChDeviceProtCutOff, current);
    ...
    processLampProtection(KoLED_RGBW_ChLampProtConstCurrent, KoLED_RGBW_ChLampProtOverload, ParamLED_RGBW_ChLampProtActive, ParamLED_RGBW_ChLampProtCableLength, ParamLED_RGBW_ChLampProtCableCrossSect, ParamLED_RGBW_ChLampProtConstPower, ParamLED_RGBW_ChLampProtOverloadPercent, ParamLED_RGBW_ChLampProtOverloadTimeMS, _lampProtOverloadTimer, ParamLED_RGBW_ChLampProtCutOff, current, voltage, 3);
}

void RGBWChannel::loop()
{
    if (!_channelActive)
        return;
    ...
    if (canDim && delayCheckMillis(_lastDimTimestamp, DIMLOOP_DELAY))
    {
        ...
        if (_pHWChannels[0] < LED_ChannelCount)
            _pDimmer->setLevel(_pDimmer->scale(rgb.Red(), (HWDimmer::DimLUTType)ParamLED_RGBW_ChDimCurve), _pHWChannels[0]);
        if (_pHWChannels[1] < LED_ChannelCount)
            _pDimmer->setLevel(_pDimmer->scale(rgb.Green(), (HWDimmer::DimLUTType)ParamLED_RGBW_ChDimCurve), _pHWChannels[1]);
        if (_pHWChannels[2] < LED_ChannelCount)
            _pDimmer->setLevel(_pDimmer->scale(rgb.Blue(), (HWDimmer::DimLUTType)ParamLED_RGBW_ChDimCurve), _pHWChannels[2]);
    ...
    // Stairway Timeout
    if (getStairTrigger() && delayCheckMillis(getStairTime(), ParamLED_RGBW_ChStairCaseTimeMS))
    {
        setStairTrigger(0);
        if (ParamLED_RGBW_ChStartupBehavior)
            setLastOnValue(_brightness.value());
        _brightness.setTargetValue(0, dimmingTimeOFF());
    }
}

void RGBWChannel::processInputKo(GroupObject& ko)
{
    Colors::HSV hsv;
    Colors::RGB rgb;
    int16_t relKO = (ko.asap() - LED_RGBW_KoOffset);
    logDebugP("processInputKo Channel");
    logHexDebugP(ko.valueRef(), ko.valueSize());

    // check if channel is valid
    if ((int8_t)(relKO / LED_RGBW_KoBlockSize) == channelIndex())
        relKO = relKO % LED_RGBW_KoBlockSize;
    else
        relKO = -1;

    if (relKO == LED_RGBW_KoChLocking)
        _isLocked = ko.value(DPT_Switch);
    else if (!_isLocked)
    {
        switch (relKO)
        {
            case LED_RGBW_KoChSwitch:
                if (!getLock())
                    setSwitch(ko.value(DPT_Switch));
                break;

            case LED_RGBW_KoChSwitchNoDim:
                if (!getLock())
                    setSwitchNoDim(ko.value(DPT_Switch));
                break;

            case LED_RGBW_KoChLocking:
                setLock(ko.value(DPT_Switch));
                KoLED_RGBW_ChStateLocking.value(getLock(), DPT_Switch);
                break;

            case LED_RGBW_KoChBrightness:
                if (!getLock())
                    setBrightness((u_int16_t)((u_int16_t)ko.value(DPT_Scaling) * VALUE_KNX_MULTIPLY));
                break;

            case LED_RGBW_KoChBrightnessRel:
                if (!getLock())
                {
                    int16_t tmpu16;
                    tmpu16 = *KoLED_RGBW_ChBrightnessRel.valueRef();
                    if (tmpu16 >= 0x09)
                        relDimUp();
            ...
            case LED_RGBW_KoChColorTemperature:
                if (!getLock())
                    setColorTemperature(ko.value(Dpt(7, 600)));
                break;

            case LED_RGBW_KoChColorTemperatureRel:
                if (!getLock())
                {
                    int16_t tmpu16;
                    tmpu16 = *KoLED_RGBW_ChColorTemperatureRel.valueRef();
                    if (tmpu16 >= 0x09)
                        relDimUpColor();
                    if (tmpu16 > 0x00 && tmpu16 < 0x08)
                        relDimDownColor();
            ...
            // Day or Night
            case LED_RGBW_KoChNight:
                if (!getLock())
                    setNight(ko.value(DPT_Switch));
                break;

            case LED_RGBW_KoChStateOnOff:
...
void RGBWChannel::handleScene(uint8_t sceneNr)
{
...
uint16_t RGBWChannel::dimmingTimeON()
{
    return getNight() ? ParamLED_RGBW_ChLightDimmNightOnTime : ParamLED_RGBW_ChLightDimmDayOnTime;
}

uint16_t RGBWChannel::dimmingTimeOFF()
{
    return getNight() ? ParamLED_RGBW_ChLightDimmNightOffTime : ParamLED_RGBW_ChLightDimmDayOffTime;
}

uint16_t RGBWChannel::dimmingTime(bool switchOn)
{
    return switchOn ? dimmingTimeON() : dimmingTimeOFF();
}

uint16_t RGBWChannel::dimmingValStartup()
{
    return ParamLED_RGBW_ChStartupBehavior ? getLastOnValue() : dimmingValMax();
}

uint16_t RGBWChannel::dimmingValMin()
{
    return ParamLED_RGBW_ChBrightnessMin * VALUE_KNX_MULTIPLY;
}

uint16_t RGBWChannel::dimmingValMax()
{
    return getNight() ? (ParamLED_RGBW_ChBrightnessMaxNight * VALUE_KNX_MULTIPLY) : (ParamLED_RGBW_ChBrightnessMaxDay * VALUE_KNX_MULTIPLY);
}

uint16_t RGBWChannel::checkMinMaxBrightness(uint16_t bright)
{
    if (bright < (ParamLED_RGBW_ChBrightnessMin * VALUE_KNX_MULTIPLY))
        bright = (ParamLED_RGBW_ChBrightnessMin * VALUE_KNX_MULTIPLY);
    if (bright > dimmingValMax())
        bright = dimmingValMax();
    return bright;
}

uint16_t RGBWChannel::dimmingValTarget(bool switchOn)
{
    return switchOn ? dimmingValStartup() : 0;
}

void RGBWChannel::setStartupColor()
{
    if (ParamLED_RGBW_ChStartupBehavior)
    {
...
uint8_t RGBWChannel::getDefaultColor()
{
    return getNight() ? ParamLED_RGBW_ChColorNight : ParamLED_RGBW_ChColorDay;
}

uint16_t RGBWChannel::checkMinMaxColorTemp(uint16_t colorTemp)
{
...
void RGBWChannel::setSwitch(bool switchOn)
{
    _sceneNumberActive = 0;
    if (switchOn)
    {
        logDebugP("switch_ON");
        _brightness.setTargetValue(ParamLED_RGBW_ChBrightnessMin * VALUE_KNX_MULTIPLY, 1);
        // in case of stairway light
        if (ParamLED_RGBW_ChStairCaseActive && ParamLED_RGBW_ChStairCaseTrigger == 0)
        {
    ...
    else
    {
        ...
        // in case of stairway light
        if (ParamLED_RGBW_ChStairCaseActive && ParamLED_RGBW_ChStairCaseTrigger == 1)
        {
            setStairTime(delayTimerInit());
            setStairTrigger(1);
        }
    ...
    logDebugP("parammaxday: %5X", (ParamLED_RGBW_ChBrightnessMaxDay * VALUE_KNX_MULTIPLY));
}

void RGBWChannel::setSwitchNoDim(bool switchOn)
{
    _sceneNumberActive = 0;
    if (switchOn)
    {
...
void RGBWChannel::setHue(uint16_t hue)
{
    _sceneNumberActive = 0;
...
void RGBWChannel::setSaturation(uint16_t saturation)
{
    _sceneNumberActive = 0;
...
void RGBWChannel::setBrightness(uint16_t bright)
{
    _sceneNumberActive = 0;
...
void RGBWChannel::setNight(bool night)
{

    if (ParamLED_RGBW_ChScenesDisableNightSw || (!ParamLED_RGBW_ChScenesDisableNightSw && _sceneNumberActive == 0))
    {
        _isNight = night;
        ...
        if (!getNight())
        {
            logDebugP("Tag");
            RGBpicker(getDefaultColor());
            if (_brightness.value() == ParamLED_RGBW_ChBrightnessMaxNight * VALUE_KNX_MULTIPLY)
                _brightness.setTargetValue(ParamLED_RGBW_ChBrightnessMaxDay * VALUE_KNX_MULTIPLY, 2 * ParamLED_RGBW_ChLightDimmDayOnTime);
        }
        else
        {
            logDebugP("Nacht");
            RGBpicker(getDefaultColor());
            if (_brightness.value() > ParamLED_RGBW_ChBrightnessMaxNight * VALUE_KNX_MULTIPLY)
                _brightness.setTargetValue(ParamLED_RGBW_ChBrightnessMaxNight * VALUE_KNX_MULTIPLY, 2 * ParamLED_RGBW_ChLightDimmNightOnTime);
        }
    }
}

void RGBWChannel::relDimUp()
{
    _sceneNumberActive = 0;
    _brightness.setTargetValue(dimmingValMax(), ParamLED_RGBW_ChLightDimmRelTime);
}

void RGBWChannel::relDimDown()
{
    _sceneNumberActive = 0;
    _brightness.setTargetValue(dimmingValMin(), ParamLED_RGBW_ChLightDimmRelTime);
}

void RGBWChannel::relDimStop()
{
    _sceneNumberActive = 0;
    _brightness.setTargetValue(_brightness.value(), 1);
}

void RGBWChannel::setColorTemperature(uint16_t colorTemp)
{
    _sceneNumberActive = 0;
    ...
    KoLED_RGBW_ChColorTemperatureStatus.value(colorTemp, Dpt(7, 600));
}

void RGBWChannel::relDimUpColor()
{
    //_boost = false;
    _sceneNumberActive = 0;
    logDebugP("relDim_UP");
...
void RGBWChannel::relDimDownColor()
{
    //_boost = false;
    _sceneNumberActive = 0;
...
void RGBWChannel::relDimStopColor()
{
    //_boost = false;
    _sceneNumberActive = 0;
...
void RGBWChannel::setRGB(uint32_t RGBvalue)
{
    _sceneNumberActive = 0;
    Colors::HSV hsv;
    Colors::RGB rgb;
...
void RGBWChannel::RGBpicker(uint8_t selection)
{
    logDebugP("color selection:%3X%", selection);
    logDebugP("Color DAY: %3X", ParamLED_RGBW_ChColorDay);
    logDebugP("Color NIGHT: %3X", ParamLED_RGBW_ChColorNight);
    switch (selection)
    {
        case 1:
            /* color rot */
            logDebugP("color rot");
            _hue.setTargetValue(0, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 2:
            /* color  orange*/
            logDebugP("color orange");
            _hue.setTargetValue(1365, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 3:
            /* color  gelb*/
            logDebugP("color gelb");
            _hue.setTargetValue(2730, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 4:
            /* color  gelbgrün*/
            logDebugP("color gelbgrün");
            _hue.setTargetValue(4095, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 5:
            /* color  grün*/
            logDebugP("color grün");
            _hue.setTargetValue(5460, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 6:
            /* color  aquamarin*/
            logDebugP("color aquamarin");
            _hue.setTargetValue(6825, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 7:
            /* color  türkis*/
            logDebugP("color türkis");
            _hue.setTargetValue(8190, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 8:
            /* color  mint*/
            logDebugP("color mint");
            _hue.setTargetValue(9555, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 9:
            /* color  blau*/
            logDebugP("color blau");
            _hue.setTargetValue(10920, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 10:
            /* color  lila*/
            logDebugP("color lila");
            _hue.setTargetValue(12285, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 11:
            /* color  rosa*/
            logDebugP("color rosa");
            _hue.setTargetValue(13650, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 12:
            /* color  violett*/
            logDebugP("color violett");
            _hue.setTargetValue(15015, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 13:
            /* color  weiß*/
            ...
            _hue.setTargetValue(0, ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(0, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 14:
            /* color  random steady*/
            logDebugP("color random steady");
            _hue.setTargetValue(random(0x3FFF), ParamLED_RGBW_ChLightDimmDayOnTime);
            _saturation.setTargetValue(1024, ParamLED_RGBW_ChLightDimmDayOnTime);
            break;
        case 15:
...
void RGBWChannel::setHSV(uint32_t HSVvalue)
{
    _sceneNumberActive = 0;
    Colors::HSV hsv;
    Colors::RGB rgb;
...
uint32_t RGBWChannel::conv_Temp2RGB(int temp)
{
    uint8_t r, g, b = 0;
    if (temp > 10000)
        return 0x000000;
...
int RGBWChannel::conv_RGB2Temp(uint32_t target_rgb)
{
...
bool RGBWChannel::getCO1()
{
    return ParamLED_RGBW_ChCO1;
}

bool RGBWChannel::getCO2()
{
    return ParamLED_RGBW_ChCO2;
}

bool RGBWChannel::getCO3()
{
    return ParamLED_RGBW_ChCO3;
}
