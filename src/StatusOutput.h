...
// for dimmable values, or while a measured value keeps crossing its change threshold).
#define STATUS_SEND_RATE_MS 1000

// Per-value bookkeeping shared by every status output of the module (one instance per status KO).
struct StatusValueState
{
...
// One consistent status-feedback mechanism for the whole OFM-LedModule (brightness, colour
// temperature, RGB/HSV, current, power, supply voltage, temperature, totals, ...).
namespace StatusOutput
{
    ...
    //     is always correct. gateSend = false keeps the value/baseline current without putting it
    //     on the bus (e.g. colour/temperature while the lamp is off).
    template <typename T>
    void sendValue(GroupObject &ko, Dpt dpt, bool enabled, bool inTransition, T value,
                   StatusValueState &state, uint32_t cyclicTimeMS,
    ...
    inline void sendSwitch(GroupObject &ko, bool enabled, bool stateOn, uint32_t cyclicTimeMS, uint32_t &cyclicTimer)
    {
        ...
        if ((bool)ko.value(DPT_State) != stateOn)
            ko.value(stateOn, DPT_State);
